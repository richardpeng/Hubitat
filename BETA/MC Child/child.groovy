 ...
 *  Changes:
 *
 *  V14.1.0 - Added 'echo speaks' option to Music Player
 ...
 state.msgType = messageAction
    if(state.msgType == "Voice Message (MusicPlayer)"){
        ...
        state.multiVolumeSlots = multiVolume
		 ...
		 if(state.multiVolumeSlots == false){
            input "volume1", "number", title: "Normal Speaker volume", description: "0-100%", required: true
        ...
        if(speakerNo != null){
         ...
         input "volumeA", "number", title: "Normal Volume (Slot 1)", description: "0-100%",   required: true
         input "speakerN2", "capability.musicPlayer", title: "Music Player Device Slot 2", required: true, multiple: true
         input "volumeB", "number", title: "Normal Volume (Slot 2)", description: "0-100%",  required: true    
        ...
        input "volumeC", "number", title: "Normal Volume (Slot 3)", description: "0-100%",  required: true
        state.voiceVolumeA = volumeA
        state.voiceVolumeB = volumeB   
        ...
        input "volumeD", "number", title: "Normal Volume (Slot 4)", description: "0-100%",  required: true      
        state.voiceVolumeA = volumeA
        state.voiceVolumeB = volumeB   
        state.voiceVolumeC = volumeC
        ...
        input "volumeE", "number", title: "Normal Volume (Slot 5)", description: "0-100%",  required: true
        state.voiceVolumeA = volumeA
        state.voiceVolumeB = volumeB   
        state.voiceVolumeC = volumeC
        state.voiceVolumeD = volumeD    
	...
	//	section("'Quiet Time' - This is to reduce volume during a specified time") {
        ...
        if(speakerNoq != null){
         ...
         input "volumeAq", "number", title: "Quiet Volume (Slot 1)", description: "0-100%",  required: true
         input "speakerN2q", "capability.musicPlayer", title: "Music Player Device Slot 2", required: true, multiple: true
         input "volumeBq", "number", title: "Quiet Volume (Slot 2)", description: "0-100%",   required: true    
        ...
        input "volumeCq", "number", title: "Quiet Volume (Slot 3)", description: "0-100%",  required: true
        state.voiceVolumeAq = volumeAq
        state.voiceVolumeBq = volumeBq   
        ...
        input "volumeDq", "number", title: "Quiet Volume (Slot 4)", description: "0-100%",  required: true      
        state.voiceVolumeAq = volumeAq
        state.voiceVolumeBq = volumeBq   
        state.voiceVolumeCq = volumeCq
        ...
        input "volumeEq", "number", title: "Quiet Volume (Slot 5)", description: "0-100%",   required: true
        state.voiceVolumeAq = volumeAq
        state.voiceVolumeBq = volumeBq   
        state.voiceVolumeCq = volumeCq
        state.voiceVolumeDq = volumeDq    
 ...
 LOGDEBUG("Running weatherChanceOfRain.. ")
  LOGDEBUG("state.weatherChanceOfRain = $state.weatherChanceOfRain") 
    ...
    state.rainFall = "Rain is not expected today"    
		...
		if(state.weatherChanceOfRain == "100"){
    state.rainFall = "It's almost guaranteed that it will rain, sometime today"    
    }
		else{
...
LOGDEBUG("state.volumeAll = $state.volumeAll - runTime = $runTime")
...
if(esDevice == true){speaker.setVolumeSpeakAndRestore(state.volumeES, state.fullPhrase)}
if(esDevice == false){
...
LOGDEBUG("state.dayCheck = $state.dayCheck - state.volumeAll = $state.volumeAll - runTime = $runTime")
...
LOGDEBUG("All OK! - Playing message 1: '$state.fullPhrase'")
	if(esDevice == true){
		LOGDEBUG("TalkSwitch - Echo Speaks - setting volume to: $state.volumeAll")
		speaker.setVolumeSpeakAndRestore(state.volumeES, state.fullPhrase)
	}
	if(esDevice == false){
     speaker.playTextAndRestore(state.fullPhrase)
...
def checkVolume(){
    ...
    state.volume = state.volumeAllq 
 	LOGDEBUG("state.volume = $state.volumeAllq")
    ...
    LOGDEBUG("Multi Volume Not used - setting volume of all speakers to $state.volume")
		...
		LOGDEBUG("state.volume = $state.volumeAll")
...
def version(){
	...
	logCheck()
	resetBtnName()
	schedule("0 0 10 ? * FRI *", updateCheck) //  Check for updates at 10 am every Friday
	checkButtons()
